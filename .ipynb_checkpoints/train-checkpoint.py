...
def main():
    ...
    for epoch in range(config.epochs):
        ...
        for step, batch in pbar:
            ...
            if args.local_rank <= 0:
                # 获取当前学习率 (DeepSpeed 返回的是一个 list，对应不同参数组)
                current_lr = model_engine.get_lr()[0] 
                # 2. 获取优化器内部参数 (如 eps, weight_decay)
                # 注意：model_engine.optimizer 是 DeepSpeed 封装后的优化器
                opt_params = model_engine.optimizer.param_groups[0]
                eps = opt_params.get('eps', 1e-8) # 使用 get 保证鲁棒性
        ...
        # Save Checkpoint
        save_path = f"./checkpoints/{dataset_dir_name}/{model_name}/epoch_{epoch}"
        print(f'保存路径是{save_path}')
