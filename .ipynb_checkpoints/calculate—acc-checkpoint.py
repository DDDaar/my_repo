...
#     # 请修改为您的实际文件路径
#     target_file = '/home/ma-user/work/lbx/VLMEvalKit/outputs/Qwen2.5-VL-3B-Instruct/Qwen2.5-VL-3B-Instruct_MMStar_exact_matching_result.xlsx'
...
def run_evaluation(file_path):
    ...
    try:
        ...
        # 筛选范围：Hit 为 0 且 Log 包含 "Failed"
        mask_failed_target = (df['hit_num'] == 0) & (df['log'].astype(str).str.contains("Failed", case=False, na=False))
        
        total_samples = len(df)
        original_hits = df[df['hit_num'] == 1].shape[0]
        target_failed_count = mask_failed_target.sum()
        ...
        print(f"总样本数: {total_samples}")
        print(f"原始正确数 (Hit=1): {original_hits} ({(original_hits/total_samples*100):.2f}%)")
        print(f"待处理错误样本 (Hit=0 & Failed): {target_failed_count}")
        ...
        for m_id, m_name in methods.items():
            ...
            # 2. 挽回失败案例
            still_failed_mask = mask_failed_target & ~df[f'is_match_m{m_id}']
            still_failed_df = df[still_failed_mask]
            still_failed_count = len(still_failed_df)

            print(f"🔍 成功挽回数量: {recovered_count} / {target_failed_count}")
            print(f"❌ 挽回失败数量: {still_failed_count} / {target_failed_count}")
            
            # 打印成功案例
            if recovered_count > 0:
                print(f"\n--- 挽回【成功】案例明细 (前 20 条) ---")
                for idx, row in recovered_df.head(20).iterrows():
                    # 修复反斜杠报错：先处理好字符串再放入 f-string
                    raw_pred_clean = str(row['prediction']).strip().replace('\n', ' ')[:]
                    print(f"[Row {idx}] 真值: {row['cleaned_answer']} | 提取: {row[f'pred_m{m_id}']}")
                    print(f"   原始预测: {raw_pred_clean}...")
                    print("-" * 60)
            
            # 打印失败案例
            if still_failed_count > 0:
                print(f"\n--- 挽回【失败】案例明细 (前 5 条) ---")
                for idx, row in still_failed_df.head(5).iterrows():
                    extracted = row[f'pred_m{m_id}'] if row[f'pred_m{m_id}'] else "未提取到"
                    raw_pred_clean = str(row['prediction']).strip().replace('\n', ' ')[:]
                    print(f"[Row {idx}] 真值: {row['cleaned_answer']} | 提取: {extracted}")
                    print(f"   原始预测: {raw_pred_clean}...")
                    print("-" * 60)

            final_acc = (original_hits + recovered_count) / total_samples * 100
            final_stats[m_name] = {"recovered": recovered_count, "final_acc": final_acc}
...
if __name__ == "__main__":
    # 请确保路径正确
    target_file = '/home/ma-user/work/lbx/VLMEvalKit/outputs/Qwen2.5-VL-3B-Instruct/Qwen2.5-VL-3B-Instruct_HRBench8K_exact_matching_result.xlsx'
    run_evaluation(target_file)